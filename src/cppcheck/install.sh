...
CPPCHECK_VERSION=${VERSION} # 'system' checks the base image first, else installs 'latest'
...
if [ ${CPPCHECK_VERSION} = "os-provided" ] || [ ${CPPCHECK_VERSION} = "system" ]; then
    if type cppcheck > /dev/null 2>&1; then
        echo "Detected existing system install: $(cppcheck version)"
        # Clean up
        clean_up
        exit 0
    fi
    ...
    if [ "$INSTALL_CMD" = "apt-get" ]; then
        echo "Installing cppcheck from OS apt repository"
    elif [ "$INSTALL_CMD" = "apk" ]; then
        echo "Installing cppcheck from OS apk repository"
    else
        echo "Installing cppcheck from OS yum/dnf repository"
    fi
    if [ $ID = "mariner" ]; then
        check_packages ca-certificates
    fi
    check_packages cppcheck
    # Clean up
    clean_up
    exit 0
fi
...
# if ([ "${CPPCHECK_VERSION}" = "latest" ] || [ "${CPPCHECK_VERSION}" = "lts" ] || [ "${CPPCHECK_VERSION}" = "current" ]) && [ "${ID}" = "ubuntu" ] && [ "${USE_PPA_IF_AVAILABLE}" = "true" ]; then
...
# packages
# curl ca-certificates g++ cmake make

# Install required packages to build if missing
if [ "${ADJUSTED_ID}" = "debian" ]; then

    check_packages curl ca-certificates g++ cmake make
...
if [ "$(echo "${CPPCHECK_VERSION}" | grep -o '\.' | wc -l)" != "2" ]; then
    requested_version="${CPPCHECK_VERSION}"
    version_list="$(curl -sSL -H "Accept: application/vnd.github.v3+json" "https://api.github.com/repos/danmar/cppcheck/tags" | grep -oP '"name":\s*"\K[0-9]+\.[0-9]+\.[0-9]+"' | tr -d '"' | sort -rV )"
    if [ "${requested_version}" = "latest" ] || [ "${requested_version}" = "lts" ] || [ "${requested_version}" = "current" ]; then
        CPPCHECK_VERSION="$(echo "${version_list}" | head -n 1)"
    else
        set +e
        CPPCHECK_VERSION="$(echo "${version_list}" | grep -E -m 1 "^${requested_version//./\\.}([\\.\\s]|$)")"
        set -e
    fi
    if [ -z "${CPPCHECK_VERSION}" ] || ! echo "${version_list}" | grep "^${CPPCHECK_VERSION//./\\.}$" > /dev/null 2>&1; then
        echo "Invalid cppcheck version: ${requested_version}" >&2
        exit 1
    fi
fi

echo "Downloading source for ${CPPCHECK_VERSION}..."
curl -sL https://github.com/danmar/cppcheck/archive/${CPPCHECK_VERSION}.tar.gz | tar -xzC /tmp 2>&1
echo "Building..."
